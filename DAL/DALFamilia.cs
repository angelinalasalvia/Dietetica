    ...
    public class DALFamilia
    {
        ...
        */
        ...
        new SqlParameter("@codPermisoPadre", idPadre)
            ...
            foreach (DataRow dr in tabla.Rows)
            {
                int tipo = Convert.ToInt32(dr["Tipo"]); // Usa el nombre correcto de la columna
                Componente componente;

                if (tipo == 1) // Si Tipo es 1, es un Permiso
                {
                    componente = new Permiso()
                    {
                        Id = Convert.ToInt32(dr["permiso_hijo"]),
                        Nombre = dr["Nombre"].ToString(),
                        Tipo = "Simple" // O cualquier lógica que tengas para Permisos
                    };
                }
                else // Si Tipo es 0, es una Familia
                {
                    componente = new Familia()
                    {
                        Id = Convert.ToInt32(dr["permiso_hijo"]),
                        Nombre = dr["Nombre"].ToString(),
                        Tipo = "Familia" // O cualquier lógica que tengas para Familias
                    };
                }
